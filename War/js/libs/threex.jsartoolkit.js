...
THREEx.JSARToolKit	= function(opts){
	...
	if( this._debug ){
		...
		debugCanvas.style.left = canvasRaster.width;
	...
	var arDetector	= new FLARMultiIdMarkerDetector(arParam, 10);
	...
	arParam.copyCameraMatrix(tmpGlMatCam, 0.1, 10000);
	this._copyMatrixGl2Threejs(tmpGlMatCam, this._trackCamera.projectionMatrix);
}
...
THREEx.JSARToolKit.prototype.update	= function()
{
	...
	try{
		...
		ctxRaster.drawImage(ARWindow, 0,0, ctxRaster.canvas.width, ctxRaster.canvas.height);
	...
	for( var idx = 0; idx < nDetected; idx++ ){
		...
		this._compensateMatrix( event.matrix );
	}
...
THREEx.JSARToolKit.prototype._compensateMatrix = function( mat ){
	
	var __compensationMatrix = new THREE.Matrix4().makeScale(1, 1, -1);  //scale in -z to swap from LH-coord to RH-coord
    __compensationMatrix.multiply(new THREE.Matrix4().makeRotationX(THREE.Math.degToRad(90)));  //rotate 90deg in X to get Y-up;
	mat.multiply( __compensationMatrix );
};